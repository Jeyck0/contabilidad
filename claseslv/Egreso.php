...
Class Egreso {
    ...
    public function editarEgresoSep($sid){
        $conectar = $this->conectar->conectar();
        $query ="SELECT e.id_egreso as m0, `id_cuenta`, e.id_cat_egresos as id_cat_egresos, e.id_factura, DATE_FORMAT(fecha, '%d/%m/%Y'),c.nombre as m1,subcat.nombre as m3,subcat.codigo as hijo_codigo,catp.nombre as m2,catp.id_padre_egreso as id_padre_egreso,catp.codigo as padre_codigo,f.id_factura as id_factura,f.folio as m5,f.fecha_emision as m6,f.fecha_pago as m7,f.tipo as m4,f.monto as m11,f.detalle as m8,p.rut as m9,p.razon_social as m10
        FROM `egresos_sep` e 
        INNER JOIN cuentas c 
        ON e.id_cuenta = c.id 
        INNER JOIN cat_egresos subcat
        ...
        ON f.id_factura=e.id_factura
        INNER JOIN proveedores p
        ON f.id_proveedor = p.id_proveedores WHERE e.id_egreso='$sid'";   
        $consultar = mysqli_query($conectar,$query);
    ...
    public function editarEgresoPie($sid){
        $conectar = $this->conectar->conectar();
        $query ="SELECT e.id_egreso as m0, `id_cuenta`, e.id_cat_egresos as id_cat_egresos, e.id_factura, DATE_FORMAT(fecha, '%d/%m/%Y'),c.nombre as m1,subcat.nombre as m3,subcat.codigo as hijo_codigo,catp.nombre as m2,catp.id_padre_egreso as id_padre_egreso,catp.codigo as padre_codigo,f.id_factura as id_factura,f.folio as m5,f.fecha_emision as m6,f.fecha_pago as m7,f.tipo as m4,f.monto as m11,f.detalle as m8,p.rut as m9,p.razon_social as m10
        FROM `egresos_pie` e 
        INNER JOIN cuentas c 
    ...
    public function updateEgresoSep($uid,$uid_cuenta,$uid_cat_egresos,$uid_factura){
        $conectar = $this->conectar->conectar();
        $query ="UPDATE egresos_sep SET id_cuenta = '".$uid_cuenta."', id_cat_egresos = '".$uid_cat_egresos."',id_factura = '".$uid_factura."' WHERE id_egreso = '".$uid."' ";
        $consultar = mysqli_query($conectar,$query);

        return $consultar;
    }

    public function updateEgresoPie($uid,$uid_cuenta,$uid_cat_egresos,$uid_factura){
        $conectar = $this->conectar->conectar();
        $query ="UPDATE egresos_pie SET id_cuenta = '".$uid_cuenta."', id_cat_egresos = '".$uid_cat_egresos."',id_factura = '".$uid_factura."' WHERE id_egreso = '".$uid."' ";
        $consultar = mysqli_query($conectar,$query);
    ...
    public function contarPie(){
        $conectar = $this->conectar->conectar();
        $query ="SELECT COUNT(id_egreso) as cid from egresos_pie";
        $consultar = mysqli_query($conectar,$query);
    ...
    public function insertarEgresosPie($id_cuenta_egreso,$id_cat_egresos_egreso,$id_factura_egreso){

        $conectar = $this->conectar->conectar();
        $query ="INSERT INTO egresos_pie (id_cuenta,id_cat_egresos,id_factura) VALUES ('".$id_cuenta_egreso."','".$id_cat_egresos_egreso."','".$id_factura_egreso."')";
        $insertar = mysqli_query($conectar,$query);
        echo $query;
    ...
    public function obtenerDatosEgresosPie(){
        ...
        while($dado = mysqli_fetch_assoc($consultar)){
            $this->egresos_pie[] = $dado;
        }

        return $this->egresos_pie;
    }
